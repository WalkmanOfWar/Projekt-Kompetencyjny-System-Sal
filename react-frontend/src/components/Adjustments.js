  ...
  { 
    ...
    function generateSingleReservationCard(reservations, allRoomReservations,indexx,dayy) {
      ...
      return (
        ...
        <button className="btn btn-primary" onClick={() => handleAddButtonClick(indexx,dayy)}> Dodaj</button> {/* Przycisk jest dodany tutaj */}
        </td>
      );
    }
            ...
            <tr key={day.value}>
              ...
              {Array.from({ length: 12 }, (_, index) => {
                ...
                if (overlappingReservations.length > 0) {
                  return generateSingleReservationCard(overlappingReservations,classSchedules,index,day.value);
                } else {
                  ...
                  if (reservation) {
                    return generateSingleReservationCard([reservation],classSchedules,index,day.value);
                  } else {
                    return <td>
                      <button className="btn btn-primary" onClick={() => handleAddButtonClick(index,day.value)}> Dodaj</button> 
  ...
  const start=startingTimeSlots[timeindex];
  const end=endingTimeSlots[timeindex+parseInt(reservation.hours)-1];
        const overlappingTime =
          (start < otherReservation.end_time &&
          otherReservation.start_time <= start)||(otherReservation.start_time < end &&
    ...
    {classSchedules
      .filter((schedule) => ((schedule.day_of_week===day&&schedule.room.name===selectedRoom&&compareTime(parseTime(startingTimeSlots[timeindex]),parseTime(schedule.start_time))===0)===false)&&findschedules(schedule,classSchedules)===false&&((timeindex+parseInt(schedule.hours))<=12))
      .map((schedule) => (
        <option key={schedule.id} value={schedule.id}>
          {schedule.room.name}/{schedule.course.name}/{schedule.start_time}-{schedule.end_time}/{schedule.user.last_name}
        </option>
      ))}
  </select>
                </div>
