...
function Room() {
  ...
  const handleRemoveFacility = (indexToRemove) => {
    setSearchFacilities(searchFacilities.filter((_, index) => index !== indexToRemove));
    setMinQuantities(minQuantities.filter((_, index) => index !== indexToRemove));
  };
  ...
  const loadRooms = async () => {
    ...
    const result = await axios.get(`http://localhost:8080/rooms/search?facilities=${facilitiesString}&minQuantities=${quantitiesString}`);
...
const handleQuantityChange = (index, event) => {
  ...
  const generateRoomsDisplay = () => {
    ...
    <div key={index}>
      ...
      />
      <button className="btn btn-danger" onClick={() => handleRemoveFacility(index)}>-</button>
    </div>
  ))}
  <button className="btn btn-primary" onClick={handleAddFacility}>Dodaj udogodnienie</button>
</div>
